"""RAG session endpoints — /x, /followup, /end."""
...
async def session_start(request: SessionStartRequest):
    ...
    # Map Firebase UID to student_id
    try:
        student_id = user_manager.get_or_create_student_id(request.firebase_uid)
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Failed to map user to student ID: {str(e)}"
        )
